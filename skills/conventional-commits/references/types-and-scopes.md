...
- `feat`
  - New user-facing capability, endpoint, flag, command, or behavior
- `fix`
  - Corrects a bug, regression, or incorrect behavior
- `refactor`
  - Restructures code without changing external behavior
- `perf`
  - Measurable gain in latency, throughput, or memory
- `chore`
  - Maintenance that fits no other type: version bumps, tooling, configuration
- `docs`
  - Documentation only: README, doc comments, guides, man pages
- `test`
  - Adds or changes tests only, with no production code change
- `build`
  - Build system, compilation, linking, packaging
- `ci`
  - CI/CD pipeline configuration
- `style`
  - Formatting, whitespace, semicolons, import order, with no logic change
- `security`
  - Hardens security: input validation, auth checks, CVE patches
- `compat`
  - Backward-compatibility shim or migration
- `i18n`
...
- Strip the common project prefix. `cerydra-core` becomes `core`. `cerydra-cli` becomes `cli`.
...
- If both a package name and an inner module fit, use the inner module. It is more precise. A rewrite of prompt templates in `cerydra-core/src/prompt.rs` gets `prompt`, not `core`.
...
- `Cargo.toml`, `Cargo.lock`, `package.json`, lockfiles, `go.mod`
  - `chore(deps)`
- `.github/workflows/*`, `.gitlab-ci.yml`, `Jenkinsfile`
  - `ci`, with no scope
- `justfile`, `Makefile`, `build.rs`, build scripts
  - `build`
- `config.toml`, `.env`, settings files
  - `chore(config)`
- `.gitignore`, `.editorconfig`, `rustfmt.toml`
  - `chore(config)`
- `README.md`, `docs/*`, `CHANGELOG.md`
  - `docs(<file name without extension>)`
- `Dockerfile`, `docker-compose.yml`, Kubernetes manifests
  - `build(docker)` or `build(k8s)`

A dependency change that adds a feature is still a `feat`. This map applies to changes that only touch these files. Each entry lists the files first and the type and scope second.
...
3. Infrastructure tier: match the non-source file map above.
